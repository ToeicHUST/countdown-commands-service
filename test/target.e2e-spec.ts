...
import { JwtAuthGuard } from '@toeichust/common';
...
describe('TargetController (e2e)', () => {
  ...
  describe('PATCH /api/target', () => {
    it('should update target successfully (Valid Data)', async () => {
      ...
      expect(response.body.data.userId).toBe(MOCK_USER_ID);
    ...
    it('should update target successfully when targetDate is null or missing', async () => {
      const payload = {
        scoreValue: 500,
        targetDate: null,
      };
